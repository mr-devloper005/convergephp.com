...
import { ArrowUpRight, ChevronDown, Download, FileText, Globe, MapPin, Search, Star, UserRound } from 'lucide-react'
      ...
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)]">
          ...
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-12 sm:py-16 lg:px-8">
            ...
            <h1 className="editable-display mt-4 max-w-3xl text-balance text-[2.5rem] font-extrabold uppercase leading-[0.98] tracking-[-0.02em] sm:text-5xl lg:text-6xl">
            ...
            <div className="mt-8 flex flex-col gap-4 border-t border-[var(--tk-line)] pt-6 sm:flex-row sm:items-center sm:justify-between">
...
function RatingLine({ post, center = false }: { post: SitePost; center?: boolean }) {
  ...
  return (
    <div className={`mt-2.5 flex items-center gap-2 ${center ? 'justify-center' : ''}`}>
      ...
      {reviewsOf(post) > 0 ? <span className="text-sm text-[var(--tk-muted)]">({reviewsOf(post)})</span> : null}
    </div>
  )
}
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col overflow-hidden`}>
      <div className="relative aspect-[16/10] overflow-hidden bg-[var(--tk-raised)]">
        ...
        <span className="editable-display absolute bottom-4 left-5 text-3xl font-bold text-white">{price || 'Open offer'}</span>
        {condition ? <span className="absolute right-4 top-4 rounded-full bg-white px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-[#24262b]">{condition}</span> : null}
      </div>
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <h2 className="editable-display text-2xl font-bold leading-tight tracking-[-0.02em]">{post.title}</h2>
        ...
        <div className="mt-5 flex items-center justify-between border-t border-[var(--tk-line)] pt-4 text-xs font-medium text-[var(--tk-muted)]">
          <span className="inline-flex items-center gap-1.5">{location ? <><MapPin className="h-3.5 w-3.5" /> {location}</> : 'View full details'}</span>
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} grid overflow-hidden sm:grid-cols-[140px_minmax(0,1fr)]`}>
      <div className="flex min-h-44 items-center justify-center overflow-hidden bg-[#24262b]">
        {avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-10 w-10 text-[var(--tk-muted)]" />}
      </div>
      <div className="flex min-w-0 flex-col justify-center p-6 text-left">
        {role ? <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--tk-accent)]">{role}</p> : null}
        <h2 className="editable-display mt-2 text-2xl font-bold tracking-[-0.02em]">{post.title}</h2>
        <p className="mt-3 line-clamp-3 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
        <span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-[var(--tk-accent)]">View profile <ArrowUpRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
